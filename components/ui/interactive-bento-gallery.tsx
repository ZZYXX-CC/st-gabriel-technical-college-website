    ...
    <>
          ...
          <div className="flex-1 p-2 sm:p-3 md:p-4 flex items-center justify-center bg-gray-50/50">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <motion.button
                  className="absolute top-2 right-2 sm:top-3 sm:right-3 p-2 rounded-full bg-gray-200/80 text-gray-700 hover:bg-gray-300/80 text-xs sm:text-sm backdrop-blur-sm shadow"
      ...
      <motion.div
        ...
        onDragEnd={(_, info) => {
          ...
          let next = { x: dockPosition.x + info.offset.x, y: dockPosition.y + info.offset.y }
          const vw = typeof window !== 'undefined' ? window.innerWidth : 0
          const vh = typeof window !== 'undefined' ? window.innerHeight : 0
          const el = dockRef.current
          const elW = el ? el.offsetWidth : 300
          const elH = el ? el.offsetHeight : 60
          const margin = 16 // safe margin from edges
          // Since the dock is centered with left-1/2 and translate-x-1/2, x/y are offsets from center
          const minX = -(vw / 2 - elW / 2 - margin)
          const maxX = (vw / 2 - elW / 2 - margin)
          const minY = -(vh / 2 - elH / 2 - margin)
          const maxY = (vh / 2 - elH / 2 - margin)
          next.x = Math.min(Math.max(next.x, minX), maxX)
          next.y = Math.min(Math.max(next.y, minY), maxY)
          setDockPosition(next)
        }}
        className="fixed z-50 left-1/2 bottom-6 sm:bottom-4 -translate-x-1/2 touch-none"
      >
        <motion.div ref={dockRef} className="relative rounded-xl bg-sky-400/20 backdrop-blur-xl border border-blue-400/30 shadow-lg cursor-grab active:cursor-grabbing max-w-[92vw] sm:max-w-none">
          <div className="flex items-center gap-2 sm:gap-0 sm:-space-x-2 px-3 py-2 overflow-x-auto sm:overflow-visible">
            {mediaItems.map((item, index) => (
              <motion.div
                key={item.id}
                onClick={(e) => { e.stopPropagation(); setSelectedItem(item) }}
                style={{ zIndex: selectedItem.id === item.id ? 30 : mediaItems.length - index }}
                className={`relative group w-10 h-10 sm:w-9 sm:h-9 md:w-10 md:h-10 flex-shrink-0 rounded-lg overflow-hidden cursor-pointer hover:z-20 ${selectedItem.id === item.id ? 'ring-2 ring-white/70 shadow-lg' : 'hover:ring-2 hover:ring-white/30'}`}
    ...
    <div className="container mx-auto px-4 py-8 max-w-5xl">
      ...
      <AnimatePresence mode="wait">
        {selectedItem ? (
          ...
          <motion.div
            className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 gap-3 auto-rows-[140px] sm:auto-rows-[90px] md:auto-rows-[70px]"
            initial="hidden"
            animate="visible"
            exit="hidden"
          ...
          >
            {items.map((item, index) => (
              <motion.div
                ...
                className={`relative overflow-hidden rounded-xl ${isTouch ? 'cursor-pointer' : 'cursor-move'} row-span-2 ${item.span}`}
